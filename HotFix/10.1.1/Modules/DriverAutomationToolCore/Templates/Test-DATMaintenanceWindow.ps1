...
try {
    # Assign before normalising to an array. Windows PowerShell 5.1 emits a JSON array as a single
    # pipeline object, so @($json | ConvertFrom-Json) would wrap all windows into one element and
    # collapse every Start/End into a space-joined value. Assign first, then coerce to an array.
    $parsed = $json | ConvertFrom-Json
    if ($null -eq $parsed) { $windows = @() }
    elseif ($parsed -isnot [System.Array]) { $windows = @($parsed) }
    else { $windows = $parsed }
} catch {
